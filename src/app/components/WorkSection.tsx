...
const amulGallery = [
  { src: '/assets/images/amul_campagine_1st.jpg', alt: 'Amul Campaign - Parlour Flavours Now At Home' },
  { src: '/assets/images/amul_campagine_2nd.jpg', alt: 'Amul Campaign - So We Let Them In' },
  { src: '/assets/images/amul_campagine_3rd.jpg', alt: 'Amul Campaign - Not Everything Is Packed' },
  { src: '/assets/images/amul_campagine_4th.jpg', alt: "Amul Campaign - Some Flavours Don't Come Home" },
  { src: '/assets/images/amul_campagine_5th.jpg', alt: 'Amul Campaign - Things Are Starting To Melt' },
];
...
const projects: Project[] = [
  {
    id: 1,
    title: 'Amul Ad Campaign',
    category: 'Branding',
    tags: ['Ad Campaign', 'Art Direction', 'Visual Design'],
    year: '2026',
    imgSrc: '/assets/images/amul_campagine_1st.jpg',
    imgAlt: 'Amul Ad Campaign - Parlour Flavours Now At Home',
  ...
  },
  {
    ...
    imgAlt: 'Poem illustration series artwork by Yash Verma with expressive visual poetry',
    colSpan: 'md:col-span-1',
    aspectClass: 'aspect-[4/5]',
  },
  {
    ...
    imgAlt: 'Product illustration artwork by Yash Verma',
    colSpan: 'md:col-span-1',
    aspectClass: 'aspect-[4/5]',
  },
];
...
export default function WorkSection() {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowRight') setGalleryIndex((i) => (i + 1) % amulGallery.length);
      if (e.key === 'ArrowLeft') setGalleryIndex((i) => (i - 1 + amulGallery.length) % amulGallery.length);
    };
    window.addEventListener('keydown', handleKey);
    return () => window.removeEventListener('keydown', handleKey);
  }, [galleryOpen]);

  // Prevent body scroll when gallery open
  useEffect(() => {
    ...
    <section id="work" ref={sectionRef} className="py-24 md:py-32 bg-background">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
          {filtered.map((project, idx) => (
            <div
              ...
              onClick={() => project.isGallery && openGallery(0)}
              ...
              <div className={`relative w-full overflow-hidden ${project.aspectClass}`}>
                ...
                {project.isGallery && (
                  <div className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 z-10">
                    ...
                    <span className="text-[11px] text-white font-medium">{amulGallery.length} images</span>
                  </div>
                )}
                ...
                <div className="card-overlay absolute inset-0 flex flex-col justify-end p-6">
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center flex-shrink-0 ml-4">
                      <Icon name={project.isGallery ? 'EyeIcon' : 'ArrowUpRightIcon'} size={18} variant="outline" className="text-white" />
                    </div>
                  </div>
                </div>
      ...
      {galleryOpen && (
        ...
        >
          {/* Close button */}
          <button
            className="absolute top-5 right-5 z-10 w-11 h-11 rounded-full bg-white/10 border border-white/20 flex items-center justify-center hover:bg-white/20 transition-colors"
            onClick={() => setGalleryOpen(false)}
          >
          ...
          <div className="absolute top-5 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-white/10 border border-white/20">
            <span className="text-white text-sm font-medium">{galleryIndex + 1} / {amulGallery.length}</span>
          </div>

          {/* Prev button */}
          <button
            className="absolute left-4 z-10 w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center hover:bg-white/20 transition-colors"
            onClick={(e) => { e.stopPropagation(); setGalleryIndex((i) => (i - 1 + amulGallery.length) % amulGallery.length); }}
          >
          ...
          {/* Image */}
          <div
            className="relative max-w-2xl w-full mx-20 aspect-[3/4]"
            onClick={(e) => e.stopPropagation()}
          >
            <img
              src={amulGallery[galleryIndex].src}
              alt={amulGallery[galleryIndex].alt}
          ...
          {/* Next button */}
          <button
            className="absolute right-4 z-10 w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center hover:bg-white/20 transition-colors"
            onClick={(e) => { e.stopPropagation(); setGalleryIndex((i) => (i + 1) % amulGallery.length); }}
          >
          ...
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            {amulGallery.map((img, i) => (
              <button
                key={i}
                onClick={() => setGalleryIndex(i)}
                className={`w-14 h-14 rounded-lg overflow-hidden border-2 transition-all ${i === galleryIndex ? 'border-primary scale-110' : 'border-white/20 opacity-60 hover:opacity-100'}`}
              >
